// Note: this file is an edited version of the one included with Chris Colefax's City Generator Include File
// Note: You have to change the scene background color from blue, otherwise the blue gets reflected off the glass and becomes visible.
#version 3.7;
...
#declare Max_Storeys = 1;		// roughly, the maximum height of a building, in storeys (actual values are random, and are slightly greater or less than this). (note: a size-1 hill is 32 units tall.)

#ifndef (Max_Storeys) #declare Max_Storeys = 2 * floor(clock * 4);	#end
#if (Max_Storeys = 0)
...
#declare Fit_To_Tile = false;		// if enabled, the scene is scaled so that it fits within the target 64-pixel wide sprite (this overrides Scale_Factor).
#declare Rotate_Y = clock * 1440;	// rotate the object around the Y-axis by this amount.
#declare City_Seed = 11999;		// seed for random city generator.
...
/*
// ENVIRONMENT
   light_source {<-300, 1000, -700> srgb 1}
   light_source {<200, 4000, 100> srgb .5 shadowless}
//   default {finish {ambient .3 diffuse .5}}

   background {srgb <.4, .6, .8>}
//   fog {fog_type 2 color srgb <.7, .8, .9> distance 300 fog_alt 50}
